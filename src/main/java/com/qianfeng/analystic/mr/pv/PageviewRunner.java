...
public class PageviewRunner implements Tool{
    ...
    public int run(String[] args) throws Exception {
        ...
        TableMapReduceUtil.initTableMapperJob(this.getScans(job),PageviewMapper.class,
                StatsUserDimension.class, TimeOutputValue.class,job,false);

        //reducer的设置
        job.setReducerClass(PageviewReudcer.class);
    ...
    private List<Scan> getScans(Job job) {
        ...
        fl.addFilter(new SingleColumnValueFilter(Bytes.toBytes(EventLogsConstant.HBASE_COLUMN_FAMILY),
                ...
                Bytes.toBytes(EventLogsConstant.EventEnum.PAGEVIEW.alias)));
        //设置扫描的字段
        String[] fields = {
          EventLogsConstant.EVENT_COLUMN_NAME_SERVER_TIME,
          EventLogsConstant.EVENT_COLUMN_NAME_CURRENT_URL,
